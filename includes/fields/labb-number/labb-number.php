...
if (!class_exists('LABB_Number_Field')) {
    ...
    class LABB_Number_Field {
        ...
        function render_input_field($name, $value, $field, $settings) {
            ...
            if (empty($value)) {
                $value = (isset($field['default'])) ? intval($field['default']) : 3; // assume 3 as default in case default is not specified
            }

            ?>

            <div class='labb-number-wrap fl-field' data-type='text' <?php echo $preview; ?>>
                <input type="number"
