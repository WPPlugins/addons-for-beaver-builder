...
function labb_entry_published($format = null) {
    ...
    if (empty($format))
        $format = esc_html__("M d, Y", 'livemesh-bb-addons');
...
function labb_get_column_class($column_size = 3, $no_margin = false) {

    $style_class = 'labb-threecol';

    $no_margin = labb_to_boolean($no_margin); // make sure it is not string
    ...
    $style_class = $no_margin ? ($style_class . ' labb-zero-margin') : $style_class;
...
function labb_get_taxonomy_terms_filter($taxonomy, $chosen_terms = array()) {
